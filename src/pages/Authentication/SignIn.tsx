...
const SignIn: React.FC<Props> = ({ setToken }: Props) => {
  ...
  const loginUser = (credentials: formDataType) => {
    return userService.login(credentials).then((res) => {
      return res.data;
    });
  };
  const setUserSession = (userData: string) => {
  ...
  const login = async (event: React.ChangeEvent<HTMLFormElement>) => {
    ...
    if (response.status == 200 && response.data.token) {
      ...
      if (response.data.user_type_name == 'admin') {
        navigate(`/users`);
      } else {
        navigate(`/dashboard`);
      }
